...
def check_ping():
    ...
    print_(('Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: '
               '%(latency)s ms' % best).encode('utf-8', 'ignore'))
    #threading.Timer(period, periodic_check_ping(period)).start()
    return ping
...
def check_download_speed():
    ...
    print_('Download: %0.2f M%s/s' %
           ((dlspeed / 1000 / 1000) * args.units[1], args.units[0]))
    return converted_download_speed
...
def check_upload_speed():
    ...
    print_('Upload: %0.2f M%s/s' %
           ((ulspeed / 1000 / 1000) * args.units[1], args.units[0]))
    return converted_upload_speed




def periodic_check(period, func):
    func()
    threading.Timer(period, periodic_check, args=(period, func)).start()


def main():
    #threading.Thread(target=periodic_check, args=(10, check_ping)).start()
    try:
        periodic_check(15, check_ping)
